<script lang="ts">
	...
	const color = (node: Node): string => {
		if (width < height) return lightColor
		return node.x > width / 2 ? darkColor : lightColor
	}

	const resizeHandler = () => {
		const toMoveAround =
			nodes[0] == undefined || canvas.width !== canvas.clientWidth * window.devicePixelRatio
	...
	onMount(() => {
		...
		function animate() {
			...
			for (let i in nodes) {
				...
				while (nodes[i].x < 0) nodes[i].x += width
				nodes[i].x %= width

				while (nodes[i].y < 0) nodes[i].y += height
				nodes[i].y %= height
				...
				for (let j in nodes) {
					...
					if ((nodes[i].x - width / 2) * (nodes[j].x - width / 2) < 0) {
						const intermediary =
							((nodes[j].y - nodes[i].y) * (width / 2 - nodes[i].x)) / (nodes[j].x - nodes[i].x) +
							nodes[i].y
